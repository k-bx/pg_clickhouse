...
ch_binary_init_convert_state(Datum val, Oid intype, Oid outtype) {
    ...
    if (intype == ANYARRAYOID) {
        ...
        get_typlenbyvalalign(
            slot->item_type, &state->typlen, &state->typbyval, &state->typalign
        );
...
init_output_convert_state(ch_convert_output_state* state) {
    ...
    state->ctype = find_coercion_pathway(
        state->outtype, state->intype, COERCION_EXPLICIT, &state->castfunc
    );
    ...
    for (i = 0; i < n; i++) {
        ...
        if (NameStr(TupleDescAttr(indesc, 0)->attname)[0] == '\0') {
            ...
            for (j = 0; j < indesc->natts; j++) {
                ...
                cinfo = OidIsValid(relid) ? chfdw_get_custom_column_info(relid, j + 1)
                                          : NULL;
                inattname = (cinfo && cinfo->colname[0]) ? cinfo->colname
...
build_nested_binary_array(
    ...
    if (level + 1 == ndim) {
        ...
        for (size_t i = 0; i < arr->len; i++) {
            ch_binary_array_t* child = build_nested_binary_array(
 ...
 */
void
ch_binary_do_output_conversion(
    ch_binary_insert_state* insert_state,
    TupleTableSlot* slot
) {
...
#else
                    array_iter_setup(
                        &iter, v, cstate->typlen, cstate->typbyval, cstate->typalign
                    );
                    for (size_t j = 0; j < total; j++) {
...
#endif
                    arr = build_nested_binary_array(
                        0, ndim, dims, cstate->innertype, flat, flatnulls, &idx
                    );
